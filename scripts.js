...
function computerPlay() {
  ...
  let computerSelection =
    playSelections[Math.floor(Math.random() * playSelections.length)];

  return computerSelection;
}
// Helper function capitalizes first letter and lower cases the rest of the string
...
function displayResult(roundResult) {
  ...
  if (computerScore === 5 || playerScore === 5) {
    if (computerScore === 5) {
      results.innerHTML += `<hr> <b>Game Over</b><br> Sorry, You Lost!<hr>`;
    } else if (playerScore === 5) {
      results.innerHTML += `<hr> <b>Game Over</b><br> Congratulations, You Won!<hr>`;
    }
